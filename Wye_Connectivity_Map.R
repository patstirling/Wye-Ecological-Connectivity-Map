...
m <- leaflet(options = leafletOptions(minZoom = 9)) %>%
  ...
  setView(lng = -3.05, lat = 52.0, zoom = 10) %>%
  ...
  addControl(
    html = paste0(
      ...
      z-index: 9999;
    '>
      <b>Wye Valley Wilding Projects overlaid with ecological connectivity analysis.</b><br>
      Ecological connectivity mapping provided by Dr Ewan McHenry (Woodland Trust).<br><br>
      <b>ECA:</b> Effective Connected Area based on modelling of ecological interconnectivity from 1990 and 2019.
    </div>"
    ),
    position = "bottomleft"
  )
